...
)
{
	...
	//TerminalSend("pf_open: Mode select\n");
        sprintf(print,"pf_open: Mode select");
	...
	if (res == FR_OK) {
		...
		if (mode & (FA_CREATE_ALWAYS | FA_OPEN_ALWAYS | FA_CREATE_NEW)) {
			...
			//TerminalSend("pf_open: Open start\n");
                  sprintf(print,"pf_open: Open start");
                  ssd1963_PutText(20,200,print,Black,White);
