...
/ If not, contiguous sectors are allocated to the file. If the file has
/ been opened without FA_WRITE flag, it only checks if the file is
/ contiguous and returns the resulut. */
...
DWORD allocate_contiguous_clusters (    /* Returns file start sector number (0:error or not contiguous) */
