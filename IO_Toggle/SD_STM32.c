...
// SSI port
#define SDC_SSI_BASE            SPI2
#define SDC_SSI_SYSCTL_PERIPH   SYSCTL_PERIPH_SSI0
// GPIO for SSI pins
#define SDC_GPIO_PORT_BASE      GPIOD
#define SDC_GPIO_SYSCTL_PERIPH  SYSCTL_PERIPH_GPIOD
#define SDC_SSI_CLK             GPIO_Pin_13
#define SDC_SSI_TX              GPIO_Pin_15
#define SDC_SSI_RX              GPIO_Pin_14
...
void SELECT (void)
{
    GPIO_ResetBits(SDC_GPIO_PORT_BASE, SDC_SSI_FSS);
}
...
void DESELECT (void)
{
    GPIO_SetBits(SDC_GPIO_PORT_BASE, SDC_SSI_FSS);
}
...
void xmit_spi(BYTE dat)
{
    ...
    ui32RcvDat=SPI_ReceiveData(SPI2);
   // DESELECT();
}
...
void send_initial_clock_train(void)
{
   ...
   // ROM_GPIOPinTypeGPIOOutput(SDC_GPIO_PORT_BASE, SDC_SSI_TX);
    ...
    GPIO_SetBits(SDC_GPIO_PORT_BASE, SDC_SSI_TX);
    ...
    for(i = 0 ; i < 10 ; i++)
    {
        ...
        SPI_SendData(SDC_SSI_BASE, 0xFF);

        /* Flush data read during data write. */
       // ROM_SSIDataGet(SDC_SSI_BASE, &ui32Dat);
        ui32Dat=SPI_ReceiveData(SDC_SSI_BASE);
    }
...
void set_max_speed(void)
{
   ...
   /* i = ROM_SysCtlClockGet() / 2;
    ...
    /* Configure the SSI1 port to run at 12.5MHz */
    //ROM_SSIConfigSetExpClk(SDC_SSI_BASE, ROM_SysCtlClockGet(),
